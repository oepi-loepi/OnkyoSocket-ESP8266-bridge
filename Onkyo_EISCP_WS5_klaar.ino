...
const char* ssid     = ""; //Change this value for your network
...
bool dhcp = false;
const char* wifiHostname = "OnkyoBridge";
...
void setup()
{
  ...
  //end read
    ...
    delay(1000);
    Sendcommand("NJADIS");   //disable jacket art      
    delay(30);
}
...
void sendjson(bool checkvoornieuw){
  jsoncontentnew = "";
  String jsoncontentnew = "{\"title\":\"" + title1 + "\"," ;
  jsoncontentnew += "\"artist\":\"" + artist + "\"," ;
  jsoncontentnew += "\"timestamp\":\"" + timestamp + "\"," ;
  jsoncontentnew += "\"power1\":\"" + power1 + "\"," ;
  jsoncontentnew += "\"volume\":\"" + volume + "\"," ;
  jsoncontentnew += "\"ip\":\"" + String(onkyoadress) + "\"," ;
  jsoncontentnew += "\"input\":\"" + input + "\"," ;
  jsoncontentnew += "\"tuner\":\"" + tuner + "\"," ;
  jsoncontentnew += "\"status\":\"Onkyo<->ESP connected\"," ;
  jsoncontentnew += "\"preset\":\"" + preset + "\"}" ;
...
void parsedata(String datastring){
  Serial.println(datastring);
      ...
      found = datastring.indexOf("!1NJA2-"); if (found>=0){
        albumart = datastring.substring(found+7);
        found =-1; 
        Serial.println(albumart);
        found =datastring.indexOf("http://"); 
        if (found>=0){
            found3 = datastring.indexOf("/", found + 7); 
            if (found3>=0){
                ipofonkyo = datastring.substring(found+7,found3);
                found = -1;
                found3 = -1;
                Serial.println("ip of Onkyo : " +  ipofonkyo);
            }
        }
      }
...
void loop(){
      ...
      if (client.available()) {
          ...
          if (((32 <= c) && (c <= 126)) || (c <= '\n')){
              ...
              if (c =='\n'){
                  //Serial.println("received from ISSCP: " + fromserver);
